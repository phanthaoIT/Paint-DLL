...
void CLine::Draw(Graphics* g)
{
	
	HINSTANCE hinstLib = LoadLibrary(L"MyDLL.dll");
	MYPROC dt = (MYPROC)GetProcAddress(hinstLib, "Duong_thang");
	if (dt != NULL)
		dt(g,pen, batdau, ketthuc);
}
void CRectangle::Draw(Graphics* g)
{

	HINSTANCE hinstLib = LoadLibrary(L"MyDLL.dll");
	MYPROC dt = (MYPROC)GetProcAddress(hinstLib, "HCN");
	if (dt != NULL)
		dt(g, pen, batdau, ketthuc);
}
void CEllipse::Draw(Graphics* g)
{

	HINSTANCE hinstLib = LoadLibrary(L"MyDLL.dll");
	MYPROC dt = (MYPROC)GetProcAddress(hinstLib, "HT");
	if (dt != NULL)
		dt(g, pen, batdau, ketthuc);
}
CShape::CShape()
